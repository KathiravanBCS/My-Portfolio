...
export default function ContactPage() {
  ...
  // Detect mobile and set initial terminal mode
  useEffect(() => {
    const isMobile = typeof window !== "undefined" && window.innerWidth < 768;
    setIsTerminalMode(!isMobile);
  }, []);
  ...
  // Scroll to bottom + focus input only when user has interacted with terminal
  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => {
    if (terminalRef.current) {
      terminalRef.current.scrollTop = terminalRef.current.scrollHeight;
    }
    if (hasUserInteracted && isTerminalMode && !isTyping) {
      inputRef.current?.focus();
    }
  }, [hasUserInteracted, isTerminalMode, isTyping]);
          ...
          >
            {isTerminalMode ? (
              <>
                ...
                <span>I'm technically deprived, need simple form</span>
              </>
            ) : (
              <>
                <IconTerminal className="w-4 h-4" />
                <span>I'm technically proficient, show me the terminal</span>
              </>
            )}
          ...
          >
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 bg-white dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800 overflow-hidden rounded-lg">
                ...
                <div
                  ref={terminalRef}
                  className="p-4 h-72 overflow-y-auto bg-gray-50 dark:bg-gray-950 font-mono text-sm font-semibold cursor-text scrollbar-thin scrollbar-thumb-pink-800/60 scrollbar-track-transparent hover:scrollbar-thumb-pink-600/80"
                  style={{ scrollbarWidth: "thin", scrollbarColor: "rgba(157,23,77,0.5) transparent" }}
                  onClick={() => {
                    handleTerminalInteraction();
                    focusInput();
                  }}
                ...
                >
                  ...
                  {commandHistory.map((entry, i) => (
                    <div key={entry.id || `cmd-${i}`} className="mb-4">
